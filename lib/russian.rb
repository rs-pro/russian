...
$LOAD_PATH.push File.join(File.dirname(__FILE__), 'russian')
...
module Russian
  ...
  def pluralize(n, *variants)
    fail ArgumentError, 'Must have a Numeric as a first parameter' unless n.is_a?(Numeric)
    fail ArgumentError, 'Must have at least 3 variants for pluralization' if variants.size < 3
    fail ArgumentError, 'Must have at least 4 variants for pluralization' if variants.size < 4 && n != n.round
